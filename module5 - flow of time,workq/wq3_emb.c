...
static void workqueue_fn(unsigned long); 
 
static DECLARE_WORK(work, workqueue_fn);
 
 
/*Workqueue Function*/
static void workqueue_fn(unsigned long arg)
{
        printk(KERN_INFO "Executing Workqueue Functionn");
}
...
static ssize_t sysfs_store(struct kobject *kobj, 
                struct kobj_attribute *attr, char *buf, size_t count)
{
        printk(KERN_INFO "Sysfs - Write!!!n");
...
static int etx_open(struct inode *inode, struct file *file)
{
        printk(KERN_INFO "Device File Opened...!!!n");
        return 0;
}
...
static ssize_t etx_read(struct file *filp, 
                char __user *buf, size_t len, loff_t *off)
{
        printk(KERN_INFO "Read functionn");
...
{
        printk(KERN_INFO "Write Functionn");
        return 0;
}
...
static int __init etx_driver_init(void)
{
        ...
        if((cdev_add(&etx_cdev,dev,1)) < 0){
            printk(KERN_INFO "Cannot add the device to the systemn");
            goto r_class;
        }
        ...
        if(sysfs_create_file(kobj_ref,&etx_attr.attr)){
                printk(KERN_INFO"Cannot create sysfs file......n");
                goto r_sysfs;
        }
